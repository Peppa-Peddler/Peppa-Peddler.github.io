...
var cuencaMap = function cuencaMap(options) {
    var self = {};
    ...
    self.init = function (feature) {
        self.svg = d3.select(self.parent_select).attr("width", self.width).attr("height", self.height);
    ...
    self.render = function (feature, deps, proj, color) {
        ...
        .scaleExtent([scale, scale]).on("zoom", function () {
            self.g.attr("transform", d3.event.transform);
        });

        var t = textures.lines().orientation("vertical", "horizontal").size(5 / scale).strokeWidth(1.25 / scale).stroke(color);

        self.svg.call(t);

        self.g2.append("path").datum(feature).attr("d", path).style("stroke", color).style("stroke-width", 2.5 / scale).style("fill", t.url());
    ...
    self.renderDepartamentos = function (deps, proj, scale) {
        ...
        self.g1.selectAll("path").data(deps.features).enter().append("path").style("fill", "rgba(17, 17, 17, 0.25)").style("stroke", "#111").style("stroke-width", 2.5 / scale).attr("id", function (d, i) {
            return i;
        }).attr("d", path);

        self.labels.selectAll("text").data(deps.features).enter().append("text").text(function (d) {
            return d.properties.name;
        }).attr('font-size', 15 / scale + 'px').attr('fill', '#fff').attr("id", function (d, i) {
...
var typed = new Typed('#landing-text', {
    strings: ["Todo el agua dulce del Perú proviene de glaciares y lluvias cada vez más escasos.", "El agua discurre por ríos, lagunas y acuíferos que confluyen y drenan su contenido en 159 cuencas, antes de perderse en el mar.", "En algunas cuencas, donde conviven la agricultura y la minería, el agua es escasa y se desarrollan conflictos sociales."],
    typeSpeed: 30,
    backDelay: 1500,
...
var topogramAgua = function topogramAgua(options) {
    var self = {};
    for (var key in options) {
        self[key] = options[key];
    }
    ...
    self.init = function () {
        self.projection = d3.geoMercator().scale(self.scale).center(self.center).translate([self.width / 2, self.height / 2]);

        self.path = d3.geoPath().projection(self.projection);

        self.cartogram = d3.cartogram().projection(self.projection).properties(function (d) {
            return d.properties;
        }).value(function (d) {
            return 1;
        });
        ...
        self.g2 = self.g.append("g");
        self.layer = self.g1.append("g").attr("id", "layer");
        self.states = self.layer.append("g").attr("id", "states").selectAll("path");

        self.loadData();

        self.zoom = d3.zoom().scaleExtent([1, 4]).on("zoom", function () {
        ...
        d3.json(self.url_topo, function (topo) {
            ...
            d3.json(self.url_deps, function (error, departamentos) {
                ...
                d3.csv(self.url_licencias, function (licencias) {
                    ...
                    self.states.attr("id", function (d, i) {
                        d.id = i;
                        return 'path-' + d.id;
                    }).attr("a", function (d) {
                    ...
                    $(".btn-go").click(function () {
                        ...
                        setTimeout(function () {
                            ...
                            setTimeout(function () {
                                self.states.transition().duration(750).ease(d3.easeLinear).attr("fill", function (d) {
                                    if (self.data.cuencas[d.properties.CODIGO] !== undefined) {
                                ...
                                setTimeout(function () {
                                    ...
                                    var key = $(this).hasClass('off') ? 'area' : 'resoluciones';
    ...
    self.renderDepartamentos = function (departamentos) {
        ...
        self.g2.selectAll("path").data(self.geodata_deps.features).enter().append("path").attr("fill", self.custom_color['default']).style("opacity", 1).style("pointer-events", "none").attr("d", self.path);
    };
    ...
    self.prerenderCuencas = function (topo) {
        ...
        self.states = self.states.data(self.cartogram.features(self.topology, self.geometries)).enter().append("path").attr("fill", self.custom_color['default']).attr("d", self.path).on('mousemove', function (d) {
            self.showTooltip(d);
    ...
    self.prepareLicencias = function (licencias) {
        ...
        licencias.forEach(function (licencia) {
            var id = licencia['CÓDIGO DE CUENCA'],
    ...
    self.updateResults = function () {
        ...
        $.each(self.data.cuencas, function (key, cuenca) {
            if ((self.departamento_selected == 'todos' || $.inArray(self.departamento_selected, cuenca.departamentos) !== -1) && (self.empresa_selected == 'todos' || $.inArray(self.empresa_selected, cuenca.empresas) !== -1)) {
    ...
    self.showInfo = function (d) {
        var info = d3.select("#cuenca-info");
        ...
        if (self.data.cuencas[d.properties.CODIGO] !== undefined) {
            ...
            var chart = bb.generate({
                ...
                data: {
                    type: "pie",
                    columns: [["Superficial", cf['Superficial']], ["Subterráneo", cf['Subterráneo']]],
                    "colors": {
                        "Superficial": "#8da0cb",
                        "Subterráneo": "#ff4b4b"
                    }
    ...
    self.renderCuencas = function (key) {
        ...
        var value = function value(d) {
            if (key == 'area') {
                ...
                if (self.data.cuencas[d.properties.CODIGO] === undefined || self.filter_dh[self.data.cuencas[d.properties.CODIGO].DH] === false) return 0;
                return self.data.cuencas[d.properties.CODIGO].nro_licencias;
            }
        },
            values = self.states.data().map(value).filter(function (n) {
            return !isNaN(n);
        }).sort(d3.ascending),
            lo = values[0],
        ...
        var scale = d3.scaleLinear().domain([lo, hi]).range([1, 700]);
        ...
        var features = self.cartogram(self.topology, self.geometries).features;
        ...
        self.states.transition().duration(750).ease(d3.easeLinear).attr("d", self.cartogram.path);
...
var topogram_agua = new topogramAgua({
    parent_id: 'map',
    width: $("#map").width(),
    height: $("#map").height()
});
